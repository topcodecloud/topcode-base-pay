package cloud.topcode.open.pay;

import cloud.topcode.open.pay.constant.PayConstant;
import cloud.topcode.open.pay.entity.*;
import cloud.topcode.open.pay.form.PayCloseForm;
import cloud.topcode.open.pay.form.PayQueryForm;
import cloud.topcode.open.pay.form.PayRefundForm;
import cloud.topcode.open.pay.form.PayRefundQueryForm;
import cloud.topcode.open.pay.plugin.wechatpay.WechatHandler;
...
import static cloud.topcode.open.pay.util.BaseUtils.*;
...
//    private AlipayHandler alipayHandler;
     ...
     * @param wechatParam 微信支付参数
     * @return 预支付信息：支付宝返回前端可调用支付表单，微信返回前端调用的url
     * @throws AlipayApiException
     * @throws UnsupportedEncodingException
     */
    public PrepayResult prepay(PayChannel payChannel, PayType payType, BaseOrder order, String code, WechatBaseParam wechatParam) throws Exception {
...
//        }
        if (payChannel == PayChannel.WECHAT) {
            ...
            if (payType == PayType.WECHAT_JSAPI_MP) {
                url = wechatHandler.createJsapiCodeUrl(order.getOutTradeNo(), wechatParam.getAppId(),wechatParam.getWechatCodeReturnUrl());
                result.setUrl(url);
                result.setMsg("OK");
                return result;
            }
            if (payType == PayType.WECHAT_JSAPI_MICRO) {
                ...
                Code2SessionResult r = wechatHandler.microPayGetOpenId(code,wechatParam.getAppId(),
                        wechatParam.getMchid(), wechatParam.getAppSecret(), wechatParam.getSerialNo(), wechatParam.getWechatPrivateKey());
                WechatClientPayParam payParam = wechatHandler.prePayJsapiMicro(PayType.WECHAT_JSAPI_MICRO, order, r.getOpenid(),
                        wechatParam.getAppId(),wechatParam.getMchid(),wechatParam.getSerialNo(), wechatParam.getWechatPrivateKey(), wechatParam.getWechatPayNotifyUrl());
                if (payParam != null) {
...
//    }
     ...
     */
    public WechatClientPayParam prepayStep2(PayChannel payChannel, PayType payType, BaseOrder order, String codeOrOpenId,
                                            WechatBaseParam wechatParam) throws Exception {
        ...
        if (payChannel == PayChannel.WECHAT) {
            if (payType == PayType.WECHAT_JSAPI_MP) {
                return wechatHandler.prePayJsapiMp(payType, order, codeOrOpenId,
                        wechatParam.getAppId(),wechatParam.getMchid(), wechatParam.getAppSecret(), wechatParam.getSerialNo(),
                        wechatParam.getWechatPrivateKey(), wechatParam.getWechatPayNotifyUrl());
            }
//            if (payType == PayType.WECHAT_JSAPI_MICRO) {
...
//            }
     ...
     * @throws Exception
     */
    public PayQueryResult payQuery(PayQueryForm form,WechatBaseParam wechatParam) throws Exception {
...
//        }
        if (form.getPayChannel() == PayChannel.WECHAT) {
            PayQueryReturn response = wechatHandler.payQuery(form.getOutTradeNo(),wechatParam.getMchid(), wechatParam.getSerialNo(), wechatParam.getWechatPrivateKey());
     ...
     * @return PayNotify
     */
    public PayNotify payNotifyWechat(PayNotice notice,WechatBaseParam wechatParam) throws Exception {
        PayNotice n = wechatHandler.payNotify(notice,wechatParam.getV3key());
     ...
     * 微信退单回调
     */
    public PayNotify payRefundNotifyWechat(RefundNotice notice,WechatBaseParam wechatParam) throws Exception {
        RefundNotice n = wechatHandler.payRefundNotify(notice,wechatParam.getV3key());
...
//    }
     ...
     * @throws AlipayApiException
     */
    public void payClose(PayCloseForm form,WechatBaseParam wechatParam) throws Exception {
        checkPayChannel(form.getPayChannel());
//        if (form.getPayChannel() == PayChannel.ALIPAY) {
//            alipayHandler.payClose(form.getOutTradeNo());
//        }
        if (form.getPayChannel() == PayChannel.WECHAT) {
            wechatHandler.payClose(form.getOutTradeNo(), wechatParam.getMchid(), wechatParam.getSerialNo(), wechatParam.getWechatPrivateKey());
        }
    }
     ...
     * @throws Exception
     */
    public PayRefundResult payRefund(PayRefundForm form,WechatBaseParam wechatParam) throws Exception {
        checkPayChannel(form.getPayChannel());
...
//        }
        if (form.getPayChannel() == PayChannel.WECHAT) {
            RefundCreateReturn refund = wechatHandler.payRefund(form.getOutTradeNo(), form.getOutRefundNo(),
                    ...
                    wechatParam.getMchid(), wechatParam.getSerialNo(), wechatParam.getWechatPrivateKey(), wechatParam.getWechatRefundNotifyUrl());
     ...
     * @return 退单结果
     */
    public PayRefundResult payRefundQuery(PayRefundQueryForm form,WechatBaseParam wechatParam) throws Exception {
        checkPayChannel(form.getPayChannel());
        PayRefundResult result = new PayRefundResult();
        result.setPayChannel(form.getPayChannel())
                .setOutRefundNo(form.getOutRefundNo())
...
//        }
        if (form.getPayChannel() != PayChannel.WECHAT) {
            RefundQueryReturn refund = wechatHandler.payRefundQuery(form.getOutRefundNo(), wechatParam.getMchid(), wechatParam.getSerialNo(), wechatParam.getWechatPrivateKey());
